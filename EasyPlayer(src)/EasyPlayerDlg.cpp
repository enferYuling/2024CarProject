...
BOOL CLivePlayerDlg::OnInitDialog()
{
	...
	if (NULL!=pVideoWindow)		pVideoWindow->channels		=1;
	...
	if (NULL != pVideoWindow && NULL!=pVideoWindow->pDlgVideo)
	{
		...
		const char url_header[1][200] = { "rtsp://admin:lwny1234@115.236.153.177:35070"};//修改地址
		int idx = 0;
...
void	CLivePlayerDlg::UpdateComponents()
{
	...
	if (NULL != pVideoWindow && NULL != pVideoWindow->pDlgVideo)
	{
		CRect rcVideo = rcClient;
		pVideoWindow->pDlgVideo[0].MoveWindow(&rcVideo);
