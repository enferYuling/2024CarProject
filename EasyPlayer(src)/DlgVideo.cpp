...
void	CDlgVideo::CreateComponents()
{
	...
	if (NULL != pEdtPassword)	pEdtPassword->SetWindowText(TEXT("lwny1234"));
	...
	//if (NULL != pBtnPreview)		pBtnPreview->SetWindowText(TEXT("播放"));
}
...
void CDlgVideo::OnBnClickedButtonPreview()
{
	...
	else
	{
		...
		if (m_ChannelId > 0)
		{
			...
			//if (NULL != pBtnPreview)		pBtnPreview->SetWindowText(TEXT("停止"));
		}
	}
}
